...
LAZY_AF_CLI_SIMPLE_TABLE = lazy.Loader('airflow.cli.simple_table',
                                       globals(),
...
def set_connection(path_to_connections):
    """Add configuration items to Airflow :class:`airflow.models.Connection`.

    """
    raw_connections = filester.get_directory_files(path_to_connections, file_filter='*.json')
    ...
    for raw_connection in raw_connections:
        ...
        with open(raw_connection, encoding='utf-8', ) as json_file:
            ...
            with LAZY_AF_UTILS.session.create_session() as session:
                state = 'OK'
                if session.query(LAZY_AF_MODELS.Connection).\
                        filter(LAZY_AF_MODELS.Connection.conn_id == new_conn.conn_id).first():
...
def list_connections() -> List[str]:
    """Return connection information from DB.
...
def delete_connection(key) -> None:
    ...
    with LAZY_AF_UTILS.session.create_session() as session:
        try:
            to_delete = (session.query(LAZY_AF_MODELS.Connection)\
                    .filter(LAZY_AF_MODELS.Connection.conn_id == key).one())
            ...
            logging.info('Successfully deleted Airflow connection with conn_id: "%s"', key)
...
def set_templated_connection(path_to_connections: str) -> None:
    ...
    :class:`airflow.models.Connection` definition in JSON format.  For example::
    ...
    """
    for path_to_variable_template in filester.get_directory_files(path_to_connections,
                                                                  file_filter='*.j2'):
...
def set_logging_connection(path_to_connections: str = None) -> None:
    """Logging configuration to Airflow :class:`airflow.models.Connection`.

    """
    if REMOTE_LOGGING:
        if not path_to_connections:
            path_to_connections = os.path.join(pathlib.Path(__file__).resolve().parents[0],
                                               'config',
                                               'templates',
                                               'connections',
                                               'logging',
        ...
        logging.info('Remote logging not enabled.  Check "AIRFLOW__LOGGING__REMOTE_LOGGING"')
