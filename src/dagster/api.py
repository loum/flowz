...
def set_templated_webserver_config(mapping: dict, path_to_config_template: str = None) -> str:
    ...
    *path_to_config_template* directory.
    ...
    if not path_to_config_template:
        path_to_config_template = os.path.join(pathlib.Path(__file__).resolve().parents[0],
                                               'config',
                                               'templates',
                                               'webserver')
...
def list_dags(quiet: bool = False) -> Iterator[LAZY_AF_MODELS_DAG.DAG]:
    ...
    """
    dagbag = LAZY_AF_MODELS.DagBag(dag_folder=DAGS_FOLDER)
    data = sorted(dagbag.dags.values(), key=lambda d: d.dag_id)

    if not quiet:
        LAZY_AF_CLI.simple_table.AirflowConsole().print_as(
            data=sorted(dagbag.dags.values(), key=lambda d: d.dag_id),
            output='table',
            mapper=lambda x: {
                'dag_id': x.dag_id,
...
def filter_dags(token: Match[str]) -> List[LAZY_AF_MODELS_DAG.DAG]:
    ...
    *token*.
    ...
    Returns the :class:`airflow.models.dag.DAG` reference or ``None`` if not found.
