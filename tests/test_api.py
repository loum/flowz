"""Unit test cases for :mod:`dagster.api`.
"""
import os
import pathlib
...
WEBSERVER_CONFIG_PATH = os.path.join(pathlib.Path(__file__).resolve().parents[1],
                                     'src',
                                     'dagster',
                                     'config',
                                     'templates',
...
def test_set_templated_webserver_config_default_template():
    ...
    expected_path = os.path.join(pathlib.Path(__file__).resolve().parents[0],
                                 'files',
                                 'webserver',
                                 'dbauth',
                                 'admin',
    ...
    msg = 'LOCAL dev generated webserver_config.py with public role "Admin" content error'
    assert received.split('\n') == expected, msg


def test_set_templated_webserver_config():
    """Test the webserver_config.py create: "dbauth" auth and public role "Admin".
    """
    # Given a mapping for the "dbauth" Airflow auth flow
    mapping = {
    ...
    # when I generate the webserver_config.py file
    received = dagster.api.set_templated_webserver_config(mapping, WEBSERVER_CONFIG_PATH)
...
def test_set_templated_webserver_config_oauth_flow():
