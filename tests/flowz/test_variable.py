"""Unit test cases for :mod:`flowz.variable`.
...
import flowz.variable  # type: ignore[import]
...
def test_delete_task_variables(  # pylint: disable=unused-argument
    ...
    key = "banana"
    received = flowz.variable.del_variable_key(key)
...
def test_reload_of_task_variables(
    ...
    received = flowz.variable.set_variables(str(VARIABLES_PATH))
...
def test_reload_of_task_variables_missing_entry(  # pylint: disable=unused-argument
    ...
    key = "task-01"
    flowz.variable.del_variable_key(key)
...
def test_get_variables(task_variables: int) -> None:  # pylint: disable=unused-argument
    ...
    received = flowz.variable.get_variable(key)
